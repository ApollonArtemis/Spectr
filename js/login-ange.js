...
document.getElementById("sendOTP").addEventListener("click", () => {
    ...
    const feedbackElement = document.getElementById('noacc-feedback'); // Move this here
    const email = emailInput.value.trim();

    if (email === "") {
        feedbackElement.textContent = '*Please enter your email.';
        emailInput.classList.add("is-invalid"); // Add validation error style
        return;
    }

    emailInput.classList.remove("is-invalid"); // Remove error styling if fixed

    // Send a password reset email using Firebase Authentication
    sendPasswordResetEmail(auth, email)
        .then(() => {
            const resetEmailModal = new bootstrap.Modal(document.getElementById('resetEmailModal'));
            resetEmailModal.show();
        })
        .catch((error) => {
            if (error.code === "auth/user-not-found") {
                feedbackElement.textContent = '*No account found with this email address.';
                feedbackElement.style.color = 'red'; // Set the text color to red for better visibility
            } else if (error.code === "auth/invalid-email") {
                feedbackElement.textContent = '*Invalid email format. Please enter a correct email.';
